...
def load_model(pretrain_model: str,
               model_name: str,
    ...
    model_path = model_dir_ / f'{pretrain_model}.pt'
...
def load_model_and_build_BNN(pretrain_model: str,
                            model_name: str,
                            model_dir: Union[str, Path] = './models',
                            dataset: Union[str,
                                            BenchmarkDataset] = BenchmarkDataset.cifar_10,
    ...
    if not isinstance(models[model_name]['gdrive_id'], list):
        ...
        # redirect_state_dict = {k+'_mu' if ('weight' or 'bias' in k) and (not 'bn' in k) else k: v for k, v in state_dict.items()}
        redirect_state_dict = {}
        for k, v in state_dict.items():
            if 'bn' not in k and 'downsample.1' not in k: 
                ...
                _k = k
            redirect_state_dict[_k] = v

        # load state_dict to model
        model = _partial_load_state_dict(model, model_name, redirect_state_dict, dataset_)
...
def _safe_load_state_dict(model: nn.Module, model_name: str,
    ...
    known_failing_models = {
        ...
        "Hendrycks2020AugMix_ResNeXt_warmup_aug_test","Hendrycks2020AugMix_ResNeXt_warmup_aug_test_0",
        "Hendrycks2020AugMix_ResNeXt_warmup_aug_test_1","Hendrycks2020AugMix_ResNeXt_warmup_aug_test_2",
        "Hendrycks2020AugMix_ResNeXt_warmup_aug_test_3","Hendrycks2020AugMix_ResNeXt_warmup_aug_test_4"
    }
...
def _partial_load_state_dict(model: nn.Module, model_name: str,
                          state_dict: Dict[str, torch.Tensor],
                          dataset_: BenchmarkDataset) -> nn.Module:
    try:
        model.load_state_dict(state_dict, strict=False)
...
def clean_accuracy(model: nn.Module,
    ...
    with torch.no_grad() and tqdm(total=n_batches, desc='Adapting: ') as t:
        for counter in range(n_batches):
            ...
            output, loss = model(x_curr)
            _acc = (output.max(1)[1] == y_curr).float().sum()
            acc += _acc
            t.set_postfix(acc=f'{_acc / batch_size:.4f}', loss=f'{loss:.4f}', refresh=True)
            t.update()
